...
void RFM69Manager::separator0(char sep0) {
    _separator0 = sep0;
}

void RFM69Manager::separator1(char sep1) {
    _separator1 = sep1;
}
...
bool RFM69Manager::loop() {
    ...
    if (receiveDone()) {
        ...
        buffer[length] = 0;
        DEBUG_MSG("[MESSAGE] raw message: %s\n", buffer);
...
bool RFM69Manager::send(uint8_t destinationID, char * name, char * value, uint8_t retries, bool requestACK) {
    ...
    char message[RF69_MAX_DATA_LEN];
        ...
        snprintf(message, RF69_MAX_DATA_LEN-1, "%s%c%s%c%d", name, _separator0, value, _separator0, _sendCount);
    #else
        snprintf(message, RF69_MAX_DATA_LEN-1, "%s%c%s", name, _separator0, value);
    #endif
